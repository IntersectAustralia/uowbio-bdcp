...
class TraversalTagLib {
	...
	def traversalTag = {attrs ->	
		...
		if (type == "file")
		{
			...
			out << "${file?.getName()}"
	        out << " <input type='checkbox' class='fileSelect' name='files' parentDir='${parent}' value='${reference}'>"
			out << "</span>"
			out << "</li>\n"
		}
		else
		{
	        ...
	        out << " <input type='checkbox' class='directorySelect' name='files' parentDir='${parent}' value='${reference}'>"
	        out << "</span>"
			if (children?.size() > 0)
			{
				out << "\n<ul>\n"
