...
qq.FineUploaderBasic = function(o){
    ...
    this._options = {
        ...
        messages: {
            ...
            rootHasBothFolderAndFile: "The baseroot could not contain both folders and files, please choose only floders at root dirctory to upload."
        },
        retry: {
            enableAuto: false,
            maxAutoAttempts: 3,
...
qq.FineUploaderBasic.prototype = {
    ...
    _uploadFileOrBlobDataList: function(fileOrBlobDataList, params, endpoint) {
        ...
        var index,
        	...
        	rootValid = this._isRootHasBothFolderAndFile(validationDescriptors);

        if (batchValid&&rootValid) {
    ...
    _isRootHasBothFolderAndFile : function(validationDescriptors){
    	
    	var errorMessage,
    		proposedNetFilesUploadedOrQueued = this._netUploadedOrQueued + validationDescriptors.length,
    		rootValid = this._options.callbacks.onValidateBatch(validationDescriptors) !== false;
    	if (rootValid) {
            if (filePaths.length > 0 & proposedNetFilesUploadedOrQueued > filePaths.length){
            	errorMessage = this._options.messages.rootHasBothFolderAndFile
            	this._batchError(errorMessage)
            	this._handler.cancelAll()
            	filePaths = new Array()
            	rootValid = false;
...
qq.DragAndDrop = function(o) {
    ...
    function traverseFileTree(entry) {
        ...
        if (entry.isFile) {//alert("filePath :"+entry.fullPath);
...
qq.FineUploader = function(o){
    ...
    qq.extend(this._options, {
        ...
        text: {
            uploadButton: 'Upload a single file or drag multiple files onto this button',
            cancelButton: 'Cancel',
            retryButton: 'Retry',
            deleteButton: 'Delete',
